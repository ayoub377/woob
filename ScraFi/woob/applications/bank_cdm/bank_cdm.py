...
class TransactionsFormatter(IFormatter):
    ...
    def start_format(self, **kwargs):
        self.output('  Label                                               OPE Date     VAL Date          Debit       Credit')
        self.output('---------------------------------------------------+------------+-----------+-------------+------------')
    ...
    def format_obj(self, obj, alias):
        ...
        return ' %s   %s %s %s %s' % (self.colored('%-50s' % label[:50], 'blue'),
...
class Appbank(CaptchaMixin, ReplApplication):
    ...
    SHORT_DESCRIPTION = "manage Credit du Maroc accounts"
