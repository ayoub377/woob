...
class TransactionsFormatter(IFormatter):
    """
    CHANGES :   removed date from mandatory fields because 
                CFG history doesn't have it.
                adapted table display to CDM Transaction object attributes.
    """
    MANDATORY_FIELDS = ('label', 'vdate')
    TYPES = ['', 'Transfer', 'Order', 'Check', 'Deposit', 'Payback', 'Withdrawal', 'Card', 'Loan', 'Bank']

    def start_format(self, **kwargs):
        self.output('  Label                                               OPE Date      VAL Date        Debit       Credit        Amount  ')
        self.output('---------------------------------------------------+------------+------------+------------+------------+--------------')
    ...
    def format_obj(self, obj, alias):
        ...
        return ' %s   %s %s %s  %s    %s' % (self.colored('%-50s' % label[:50], 'blue'),
                                   ...
                                   self.colored('%10.0f' % debit, 'red'),
                                   self.colored('%10.0f' % credit, 'green'),
                                   self.colored('%10.0f' % amount, 'blue'))
...
class Appbank(CaptchaMixin, ReplApplication):
    ...
    SHORT_DESCRIPTION = "manage CFG accounts"
