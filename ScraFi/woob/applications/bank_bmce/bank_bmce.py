...
class TransactionsFormatter(IFormatter):
    """
    CHANGES :   removed amount from mandatory fields because 
                BMCE history doesn't have it.
                adapted table display to BMCE Transaction object attributes.
    """
    MANDATORY_FIELDS = ('label', 'date')
    TYPES = ['', 'Transfer', 'Order', 'Check', 'Deposit', 'Payback', 'Withdrawal', 'Card', 'Loan', 'Bank']

    def start_format(self, **kwargs):
        self.output('  Label                                                 Date            Debit       Credit')
        self.output('---------------------------------------------------+------------+-------------+-------------')
    ...
    def format_obj(self, obj, alias):
        ...
        #     label = obj.raw or ''
        date = obj.date.strftime('%d-%m-%Y') if not empty(obj.date) else ''
        debit = obj.debit or Decimal('0')
        credit = obj.credit or Decimal('0')

        return ' %s   %s %s %s' % (self.colored('%-50s' % label[:50], 'blue'),
                                   self.colored('%-12s' % date[:12], 'magenta'),
...
class Appbank(CaptchaMixin, ReplApplication):
    ...
    SHORT_DESCRIPTION = "manage BMCE accounts"
