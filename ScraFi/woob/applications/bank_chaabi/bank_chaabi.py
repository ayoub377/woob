...
class TransactionsFormatter(IFormatter):
    """
    CHANGES :   removed date from mandatory fields because Chaabi history doesn't have it.
                adapted table display to Chaabi Transaction object attributes.
    """
    MANDATORY_FIELDS = ('label', 'vdate')
    TYPES = ['', 'Transfer', 'Order', 'Check', 'Deposit', 'Payback', 'Withdrawal', 'Card', 'Loan', 'Bank']

    def start_format(self, **kwargs):
        self.output(' OPE Date     VAL Date      Label                                       Ref         Dedit          Credit             Solde ')
        self.output('------------+------------+-------------------------------------------+-----------+--------------+-------------+-------------')
    ...
    def format_obj(self, obj, alias):
        ...
        debit = obj.debit or '---'
        credit = obj.credit or '---'
        ref = obj.ref or ''
        amount = obj.amount or Decimal('0')

        return ' %s %s %s %s %s %s %s ' % (self.colored('%-12s' % odate[:12], 'magenta'),
                                            self.colored('%-12s' % vdate[:12], 'yellow'),
                                            self.colored('%-43s' % label[:50], 'blue'),
                                            self.colored('%-11s' % ref[:10], 'blue'),
                                            self.colored('%-15s' % debit[:10], 'red'),
                                            self.colored('%-13s' % credit[:10], 'green'),
...
class Appbank(CaptchaMixin, ReplApplication):
    ...
    SHORT_DESCRIPTION = "manage Banque Populaire accounts"
