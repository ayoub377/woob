...
class TransactionsFormatter(IFormatter):
    """
    CHANGES :   removed amount and date from mandatory fields because 
                CIH history doesn't have them.
                adapted table display to CIH Transaction object attributes.
    """
    MANDATORY_FIELDS = ('label', 'vdate')
    TYPES = ['', 'Transfer', 'Order', 'Check', 'Deposit', 'Payback', 'Withdrawal', 'Card', 'Loan', 'Bank']

    def start_format(self, **kwargs):
        self.output('  Label                                               OPE Date       VAL Date        Ref             Debit       Credit ')
        self.output('---------------------------------------------------+-------------+------------+-----------+----------------+------------')
    ...
    def format_obj(self, obj, alias):
        ...
        #     label = obj.raw or ''
        odate = obj.odate.strftime('%d-%m-%Y') if not empty(obj.odate) else ''
        vdate = obj.vdate.strftime('%d-%m-%Y') if not empty(obj.vdate) else ''
        ref = obj.ref or ''
        debit = obj.debit or Decimal('0')
        credit = obj.credit or Decimal('0')

        return ' %s   %s %s   %s %s %s' % (self.colored('%-50s' % label[:50], 'blue'),
                                   self.colored('%-12s' % odate[:12], 'magenta'),
                                   self.colored('%-12s' % vdate[:12], 'yellow'),
                                   self.colored('%-12s' % ref, 'yellow'),
                                   self.colored('%10.2f' % debit, 'red'),
                                   self.colored('%10.2f' % credit, 'green'))
...
class Appbank(CaptchaMixin, ReplApplication):
    ...
    SHORT_DESCRIPTION = "manage CIH accounts"
    ...
    def show_history(self, command, line):
        id, start_date, end_date = self.parse_command_args(line, 3, 1)
        ...
        for transaction in self.do(command, account, backends=account.backend, **{'start_date': start_date, 'end_date': end_date}):
