...
from .browser import CAMBrowser


__all__ = ['CAMModule']


class CAMModule(Module, CapBank):
    NAME = 'cam'
    DESCRIPTION = 'Credit Agricole du Maroc'
    ...
    def iter_history(self, _id, **kwargs):
        if datetime.strptime(kwargs['start_date'], "%d/%m/%Y") < datetime.today() - relativedelta(months=3, days=2):
            kwargs['start_date'] = (datetime.today() - relativedelta(months=3, days=1)).strftime('%d/%m/%Y')
